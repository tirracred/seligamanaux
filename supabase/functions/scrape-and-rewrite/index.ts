index_ts = r"""// /supabase/functions/scraper/index.ts
// Deno 1.44+ (Supabase Edge Functions)
// Objetivos:
// - Autenticação obrigatória (JWT) + allowlist de e-mails (ALLOWED_ADMIN_EMAILS)
...
async function fetchDocument(url: string) {
  ...
  // @ts-ignore - garantir URL disponível
  (doc as any).URL = url;
  return doc;
}
...
const GROQ_MODEL = Deno.env.get("GROQ_MODEL") || "llama-3.1-70b-specdec"; // ajuste conforme disponível
...
function getSupabaseAnon() {
  ...
  if (!url || !key) throw new Error("Configure SUPABASE_URL e SUPABASE_ANON_KEY para validar JWT.");
  return createClient(url, key, { auth: { persistSession: false } });
}
...
Deno.serve(async (req) => {
  ...
  // ---- Auth obrigatória + allowlist ----
  const authHeader = req.headers.get("Authorization") || "";
  const token = authHeader.startsWith("Bearer ") ? authHeader.slice(7) : "";
  if (!token) {
    return new Response(JSON.stringify({ error: "Não autorizado: JWT ausente." }), { status: 401, headers: CORS_HEADERS });
  }
  try {
    const supabaseAuth = getSupabaseAnon();
    const { data: userResp, error: userErr } = await supabaseAuth.auth.getUser(token);
    if (userErr || !userResp?.user) {
      return new Response(JSON.stringify({ error: "Não autorizado: JWT inválido." }), { status: 401, headers: CORS_HEADERS });
    }
    const email = userResp.user.email?.toLowerCase() || "";
    const allow = (Deno.env.get("ALLOWED_ADMIN_EMAILS") || "")
      .split(",")
      .map((e) => e.trim().toLowerCase())
      .filter(Boolean);
    if (allow.length > 0 && !allow.includes(email)) {
      return new Response(JSON.stringify({ error: "Proibido: e-mail não autorizado." }), { status: 403, headers: CORS_HEADERS });
    }
  } catch (e) {
    return new Response(JSON.stringify({ error: "Falha ao validar token." }), { status: 401, headers: CORS_HEADERS });
  }
  ...
  for (const url of links) {
    try {
      ...
      if (await alreadyExists(admin, url)) continue;
      ...
      const { error } = await admin.from("noticias_scraped").insert(row);
      if (error) {
  ...
  return new Response(JSON.stringify(summary, null, 2), { headers: CORS_HEADERS });
});
